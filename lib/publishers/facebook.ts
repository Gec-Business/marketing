...
export async function postToFacebook(
  ...
  const data = await res.json();
  if (data.error) throw new Error(`Facebook: ${data.error.message}`);
  if (!data.id) throw new Error('Facebook: No post ID returned from API');
...
export async function postVideoToFacebook(
  ...
  if (!res.ok) throw new Error(`Facebook video API error: ${res.status} ${res.statusText}`);
  const data = await res.json();
  if (data.error) throw new Error(`Facebook video: ${data.error.message}`);
  if (!data.id) throw new Error('Facebook video: No post ID returned from API');
  return { postId: data.id };
}
